...
async function gatherQuotationData() {
    ...
    try {
        ...
        const result = await apiSaveQuotation(custPhone, itemsData, paymentMethod, transportMode, "BTK");
...
function drawHeader(doc, data) {
   ...
   // Header
    ...
    if (typeof LOGO_BASE64 !== 'undefined' && LOGO_BASE64.length > 100) {
        doc.addImage(LOGO_BASE64, 'PNG', logoX, 15, 26, 26);
    } else {
        doc.setFontSize(12);
    ...
    doc.text("We are pleased to provide our quotation for new tyre services.", 14, currentY);
